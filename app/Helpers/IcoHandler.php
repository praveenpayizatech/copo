...
class IcoHandler
{
        ...
        try {
            if(serverOpenOrNot()){
                ...
                if($result->status == true && $this->cris_cros($this->getDomain(), $result->valid)){
                    ...
                    return back()->with(['msg' => 'success', 'message' => '$result->message', 'data' => ['']]);
    ...
    {
        //return is_admin() ?  config('session.timeout') : __('Currently we are facing some technical issue, please try again after sometime.');
    }
